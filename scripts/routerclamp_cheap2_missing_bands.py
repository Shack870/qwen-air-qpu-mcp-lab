...
COMMON = {
    ...
    "source": "routerclamp-cheap2-missing-bands",
    "timeout_seconds": 900,
}
...
def main() -> None:
    stop_codex_gpu_helper()
    try:
        print(json.dumps({"event": "cheap2_missing_bands_start"}), flush=True)
        ranges = [
            "6:12",
            "12:18",
            "30:36",
        ...
        print("=== CHEAP2 MISSING BANDS ===", flush=True)
        for row in summaries:
            print(
                f"{row['ranges']:<16} pass={row['strict_passes']}/{row['strict_total']} "
                f"gen={row['speed_gen_tps']} pp={row['speed_pp_tps']} failed={row['failed']}",
                flush=True,
            )
        print(json.dumps({"event": "cheap2_missing_bands_done", "summaries": summaries}, sort_keys=True), flush=True)
    finally:
        resume_codex_gpu_helper()
